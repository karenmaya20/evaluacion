...
function creaOrdenar() {
	...
	for (i = 0; i < reactivosMostrar; i++) {
		...
		// preg = [];  
		// pregCheck = preg.join("@"); //con este validaremos con no queden iguales despues de sortear
		// for (j = 0; j < tmp.length; j++) {
		// 	preg.push([tmp[j], j]); // texto, numero del orden, este se sortea
		// 	pregCheck.push([tmp[j], j]); // texto, numero del orden, tal como estaba s de sortear
		// }
		...
		// 	//console.log('preg '+	preg.join()+ ' pregCheck '+pregCheck.join());
		// 	if (preg.length > 1) { //RAAR Jun 28,18: No deberia haber longitudes menores a dos....
		// 		reordenaArreglo(preg); // 
		// 	} else {break;}
		// 	//if (preg.join()==pregCheck.join()) { alert("iguales"); }
		// 	//console.log('preg '+	preg.join()+ ' pregCheck '+pregCheck.join());			 
		// }	
		var txt = "";
		for (j = 0; j < preg.length; j++) { // RAAR Jun 28,18:data-orden es para evaluar, data-despliegue es para saber si ha movido algo, debugguear
			...
			// txt += "<li class='ui-state-default ui-sortable-handle' data-txt='" + preg[j][0] + "' data-orden=" + preg[j][1] + " data-despliegue=" + j + ">" + preg[j][0] + "</li>";
		}

		if (debug) {txt += debugRespuesta} // SOLO PARA DEBUG
		// jq321('#ordenar').append("<div class='lista' id='lista"+i+"'><ul class='sortable' id='ulId" + i + "'>" + txt + tam(preguntas[i].Q, 0) + '<span data-toggle="tooltip" data-placement="auto left" data-type="success" title="' + tam(reactivos[i].F[0], 1) + '">' + palomita + '</span><span data-toggle="tooltip" data-placement="auto left" data-type="danger" title="' + tam(reactivos[i].F[1], 1) + '">' + tache + '</span>');
		jq321('#ordenar').append("<div id='lista" + i + "'><ul class='sortable' id='ulId" + i + "'>" + txt + tam(valorInicial, 0) + '<span data-toggle="tooltip" data-placement="auto left" data-type="success" title="' + tam(reactivos[i].F[0], 1) + '">' + palomita + '</span><span data-toggle="tooltip" data-placement="auto left" data-type="danger" title="' + tam(reactivos[i].F[1], 1) + '">' + tache + '</span>');
...
function creaOM(mostrar) {
	...
	for (i = 0; i < mostrar; i++) {
		...
		for (j = 0; j < reactivos[i].A.length; j++) {
			...
			jq321(".opciones:last").append('<div class="opcion btn btn-active" data-correcta="' + reactivos[i].A[j].correcta + '">' + res + tam(reactivos[i].A[j].opcion, 1));
...
function mostrarMensaje(clase, recurso) {
	...
	var msgs = [,
		...
		[ic("satnugerp sal sodot anedro ,rovaf roP"), ic("snoitseuq lla tros ,esaelP")],                            // ordenar
